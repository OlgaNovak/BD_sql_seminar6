...
/* Создайте таблицу users_old, аналогичную таблице users. Создайте процедуру, с помощью которой можно переместить
 любого (одного) пользователя из таблицы users в таблицу users_old. (использование транзакции с выбором commit
 ...
 DELIMITER //
 CREATE PROCEDURE transfer (id INT)
    BEGIN
       INSERT INTO users_old (firstname, lastname, email) (SELECT firstname, lastname, email FROM users u WHERE u.id=id);
...
BEGIN
   DECLARE mess VARCHAR(45);
   DECLARE TimeNOW TIME;
   SELECT CASE 
                           WHEN TimeNOW BETWEEN '06:00' AND '12:00' THEN 'Доброе утро'
                           WHEN TimeNOW BETWEEN '12:00' AND '18:00' THEN 'Добрый день'
                           WHEN TimeNOW BETWEEN '18:00' AND '00:00' THEN 'Добрый вечер'
                           WHEN TimeNOW BETWEEN '00:00' AND '06:00' THEN 'Доброй ночи'
	END INTO mess;
	RETURN mess;
END //
DELIMITER ;
